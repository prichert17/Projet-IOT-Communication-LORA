...
#include "/home/mflores/github/thingsat/riot_modules/modules/meshtastic_utils/include/meshtastic_utils.h"
#include "fmt.h"
...
void filter_and_decode_custom(char *buffer, size_t length);
void decode_message(char *buffer, size_t length) {
    
    uint8_t *frame = (uint8_t *)buffer;
    uint8_t frame_len = (uint8_t)length;

    printf("\n--- Analyse Meshtastic (%d bytes) ---\n", frame_len);

    printf("Données Brutes (Hex): ");
    for (size_t i = 0; i < length; i++) {
        printf("%02X ", frame[i]);
    }
    printf("\n");

    if (!meshtastic_check_valid_frame_size(frame, frame_len)) {
        printf("[!] Erreur: Trame trop courte pour être du Meshtastic (< 16 octets)\n");
        //return;
    }

    meshtastic_printf(frame,17);

    uint8_t payload[256]; 
    uint8_t payload_size = 0;

    meshtastic_get_pb_payload(frame, frame_len, payload, &payload_size);

    printf("Payload Protobuf extrait (%d octets) : ", payload_size);
...
/*void decode_message(char *buffer, size_t length) {
    printf("Données Brutes (Hex): ");
    for (size_t i = 0; i < length; i++) {
        // Affiche chaque octet en Hexa (ex: 0A FF 42)
...
int send_custom_cmd(int argc, char **argv)
{
    if (argc <= 1) {
        puts("usage: send_custom <payload>");
        return -1;
    }

    // 1. Création de l'instance de la structure
    struct custom_mesh my_packet;
    ...
    char *text_payload = argv[1];
    
    // Ahora strlen funciona porque text_payload es un puntero
    uint32_t text_len = strlen(text_payload);

    // 2. Remplissage du paquet
    // Pasamos (uint8_t*)text_payload porque la función espera una dirección de memoria
    size_t packet_size = create_custom_message(&my_packet, 
                                               my_src_id, 
                                               target_dest_id, 
                                               my_role, 
    ...
    printf("Création paquet Custom: Src=%08lX Dest=%08lX Taille=%lu\n", 
           (unsigned long)my_packet.src, (unsigned long)my_packet.dest, (unsigned long)packet_size);

    // 3. Préparation pour l'envoi
    iolist_t iolist = {
        .iol_base = &my_packet,
        .iol_len = packet_size
    };
...
void filter_and_decode_custom(char *buffer, size_t length) {
    ...
    // 3. FILTRAGE : Est-ce que ce message est pour moi ?
    // On accepte si Dest == MOI ou Dest == BROADCAST
    if (paquet->dest != MY_NODE_ID && paquet->dest != BROADCAST_ID) {
        printf("[Ignoré] Paquet pour %08lX (Je suis %08X)\n", 
               (unsigned long)paquet->dest, MY_NODE_ID);
        return;
    }

    // Si on arrive ici, c'est que le message est pour nous !
    printf("\n=== MESSAGE REÇU ===\n");
    
    if (paquet->dest == BROADCAST_ID) {
        printf("Type: BROADCAST\n");
    } else {
        printf("Type: DIRECT (Pour moi)\n");
    }

    printf("Source: 0x%08lX\n", (unsigned long)paquet->src);
    printf("Sauts : %d\n", paquet->nb_saut);

    // 4. DÉCODAGE TEXTE (ASCII)
    // On sécurise la lecture pour ne pas dépasser la taille du buffer
    uint32_t payload_len = paquet->taille;
    if (payload_len > 237) payload_len = 237; // Sécurité max

    // Création d'un buffer temporaire pour l'affichage string (+1 pour le \0)
    char text_buffer[238]; 
    
    // Copie des données
    memcpy(text_buffer, paquet->pb_payload, payload_len);
    
    // IMPORTANT : On ajoute le caractère nul à la fin pour que printf sache où s'arrêter
    text_buffer[payload_len] = '\0';

    printf("Message : \"%s\"\n", text_buffer);
    printf("====================\n");
}
int abc_cmd(int argc, char **argv)
{
    (void)argc;
    (void)argv;
	
    char *init[] = {""};
    char *setup[] = {" ","125","11","5",NULL};
    char *crc[] = {" ","set","1"};
    char *syncword[] = {" ","set","cc"};
    char *channel[] = {" ","set","869462500"};
    //char *listen[] = {""};
    //char *message_custom[] = {"Salut"};
    init_sx1272_cmd(1,init);
    lora_setup_cmd(4,setup);
    crc_cmd(3,crc);
    syncword_cmd(3,syncword);
    channel_cmd(3,channel);
    //listen_cmd(1,listen);
   // send_custom_cmd(2, message_custom);     
...
static const shell_command_t shell_commands[] = {
    ...
    { "abc",      "config initial         ",                 abc_cmd },
    {"send_custom", "send info custom sans mesh",send_custom_cmd},
